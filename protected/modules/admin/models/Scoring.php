...
class Scoring extends CActiveRecord {
    ...
    public function rules() {
        return array(
            ...
            array('status, user_id, add_article', 'numerical', 'integerOnly' => true),
            array('title, note', 'length', 'max' => 255),
            array('lang', 'length', 'max' => 2),
            array('add_article', 'length', 'max' => 1),
    ...
    public function attributeLabels() {
        return array(
            ...
            'add_article' => Yii::t("translation", "add_article"),
        );
    }
...
//            'condition' => "lang='" . Yii::app()->language . "'",
    ...
    protected function beforeSave() {
        ...
        $this->end_time = date("Y-m-d 23:59:59", strtotime($this->end_time));
    ...
    public function getAllScoringDescriptionForExport() {
        $criteria = new CDbCriteria;
        $criteria->condition = 'scoring_id = :ScoringID';
        $criteria->params = array(':ScoringID' => $this->id);
        $models = ScoringDescription::model()->findAll($criteria);
        return $models;
    }

    public function getAllScoringImageUploadForExport() {
        $criteria = new CDbCriteria;
        $criteria->condition = 'scoring_id = :ScoringID';
        $criteria->params = array(':ScoringID' => $this->id);
        $models = ScoringImageUpload::model()->findAll($criteria);
        return $models;
    }
