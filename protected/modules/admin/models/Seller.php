...
class Seller extends CActiveRecord {
    ...
    public function rules() {
        return array(
            ...
            array('email, password, role, name, contact_name, phone, note, registration_code, recover_password, team_name', 'length', 'max' => 255),
            array('lang', 'length', 'max' => 2),
            array('date_entry', 'safe'),
            array('id, email, password, role, status, create_time, name, contact_name, phone, note, date_entry, registration_code, recover_password, team_id, who_created, lang, team_name', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function attributeLabels() {
        return array(
            ...
            'team_name' => Yii::t("translation", "team_name"),
        );
    }
    ...
    public function beforeDelete() {
        ...
        $models = ScoringSeller::model()->resetScope()->findAllByAttributes(array('user_id' => $this->id));
        if (!empty($models)) {
    ...
    protected function beforeSave() {
        if ($this->isNewRecord) {
            ...
            $this->role = 'seller';
            if ($this->scenario == 'import_seller') {
                if (!empty($this->team_name)) {
                    $team = TeamName::model()->findByAttributes(array('title' => $this->team_name));
                    if (!empty($team)) {
                        $this->team_id = $team->id;
                    } else {
                        $team = new TeamName;
                        $team->title = $this->team_name;
                        if ($team->save()) {
                            $this->team_id = $team->id;
                        }
                    }
                } else {
                    $default_team = TeamName::model()->findByAttributes(array('title' => 'Default team'));
                    if (!empty($default_team)) {
                        $this->team_id = $default_team->id;
                    } else {
                        $default_team = new TeamName;
                        $default_team->title = "Default team";
                        if ($default_team->save()) {
                            $this->team_id = $default_team->id;
                        }
                    }
                }
                $model_user = User::model()->resetScope()->findByPk(Yii::app()->params['choose_customer']);
                $this->lang = $model_user->lang;
            } else {
    ...
    public function afterSave() {
        $team = Team::model()->findByAttributes(array('user_id' => $this->id));
        if (empty($team)) {
            $team = new Team;
            $team->user_id = $this->id;
        }
        $team->team_name_id = $this->team_id;
        $team->save();
    ...
    public function checkUnique($attribute, $params) {
        if (!$this->hasErrors()) {
            $model = self::model()->resetScope()->findByAttributes(array('email' => $this->email));
            if (!empty($model)) {
                $this->addError($attribute, '"' . $this->email . '" ' . Yii::t("translation", "has_already_been_taken"));
            }
        }
    }
